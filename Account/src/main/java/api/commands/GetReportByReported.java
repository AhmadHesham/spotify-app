...
public class GetReportByReported extends Command {
    @Override
    public void execute() {
        try {
             dbConn = PostgresConfig.getDataSource().getConnection();

            func = dbConn.prepareStatement("SELECT * from get_report_reported(?);");

            func.setInt(1, Integer.parseInt(map.get("reported_id")));
